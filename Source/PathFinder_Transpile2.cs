...
{
    public class PathFinder_Transpile2
    {
        public static IEnumerable<CodeInstruction> FindPath(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
			...
			LocalBuilder local_disallowedCornerIndices = iLGenerator.DeclareLocal(typeof(List<int>));
			...
			yield return new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(PathFinder_Patch), "getDisallowedCornerIndices"));
			yield return new CodeInstruction(OpCodes.Stloc, local_disallowedCornerIndices.LocalIndex);
			...
			*/
			...
			while (i < instructionsList.Count)
			{
				...
				)
				{
					...
					i + 4 < instructionsList.Count &&
					instructionsList[i+4].opcode == OpCodes.Call &&
					(MethodInfo)instructionsList[i+4].operand == AccessTools.Method(typeof(PathFinder), "CalculateAndAddDisallowedCorners")
				)
				{
					instructionsList[i].opcode = OpCodes.Ldloc;
					instructionsList[i].operand = local_disallowedCornerIndices.LocalIndex;
					yield return instructionsList[i];
					yield return new CodeInstruction(OpCodes.Ldarg_0);
					instructionsList[i+1].opcode = OpCodes.Ldfld;
					instructionsList[i+1].operand = AccessTools.Field(typeof(PathFinder), "map");
					yield return instructionsList[i+1];		
					instructionsList[i+4].operand = AccessTools.Method(typeof(PathFinder_Patch), "CalculateAndAddDisallowedCorners2");
...
)
				{
					...
					instructionsList[i].opcode == OpCodes.Ldfld &&
					(FieldInfo)instructionsList[i].operand == AccessTools.Field(typeof(PathFinder), "disallowedCornerIndices")
)
				{
					instructionsList[i].opcode = OpCodes.Ldloc;
					instructionsList[i].operand = local_disallowedCornerIndices.LocalIndex;
